...
public class OrdinaryExceptionMatcher<T extends Throwable> extends TypeSafeDiagnosingMatcher<Class<T>> {
	...
	protected boolean matchesSafely(Class<T> item, Description mismatchDescription) {
		...
		try {
			...
			mismatchDescription.appendText(item.getSimpleName() + "() is not defined or not accessible ");
			return false;
		}
		try {
			T exception = item.getConstructor(String.class, Throwable.class).newInstance("msg", new RuntimeException());
			if (!(exception.getCause() instanceof RuntimeException)) {
				mismatchDescription.appendText(item.getSimpleName() + "(String, Throwable) does not preserve cause: " + (exception.getCause() == null ? "null" : exception.getCause().getClass().getSimpleName()));
				return false;
			}
			...
			mismatchDescription.appendText(item.getSimpleName() + "(String, Throwable) is not defined or not accessible ");
			return false;
		}
		try {
			T exception = item.getConstructor(String.class).newInstance("msg");
			if (exception.getCause() != null) {
				mismatchDescription.appendText(item.getSimpleName() + "(String) has cause that is not null: " + (exception.getCause() == null ? "null" : exception.getCause().getClass().getSimpleName()));
				return false;
			}
			if (!"msg".equals(exception.getMessage())) {
			...
			mismatchDescription.appendText(item.getSimpleName() + "(String) is not defined or not accessible ");
			return false;
		}
		try {
			T exception = item.getConstructor(Throwable.class).newInstance(new RuntimeException());
			if (!(exception.getCause() instanceof RuntimeException)) {
				mismatchDescription.appendText(item.getSimpleName() + "(Throwable) does not preserve cause: " + (exception.getCause() == null ? "null" : exception.getCause().getClass().getSimpleName()));
				return false;
			}
			...
			mismatchDescription.appendText(item.getSimpleName() + "(Throwable) is not defined or not accessible ");
			return false;
		}
		return true;
	}
